...
main :: proc() {
	shaders_dir, shaders_dir_err := filepath.join({#directory, "../shaders"})
	
	assert(shaders_dir_err == nil)
	
	defer delete(shaders_dir)
...
run_compiler:: proc(file_name, input_directory : string, stage: stage ) {
		...
		output_path, output_path_err := filepath.join({#directory, "../.generated/", output_name})
		assert(output_path_err == nil)
		defer delete(output_path)

		slangc_path, slangc_err := filepath.join({#directory, "../../.dependencies/slang/bin/slangc"})
		
		assert(slangc_err == nil)
		
		defer delete(slangc_path)
